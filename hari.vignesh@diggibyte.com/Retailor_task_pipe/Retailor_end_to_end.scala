...
// MAGIC %py
// MAGIC #data = spark.read.format("csv").option("header", "true").load("dbfs:/mnt/tf-abfss/data/ds/USA_Real_Estate_HariVignesh/realtor-data.csv")
// MAGIC #display(data)
...
// MAGIC %python
// MAGIC #create mount point for silver
// MAGIC ##dbutils.fs.mount(
// MAGIC #    source = "wasbs://cntexapure@stexapure.blob.core.windows.net/silver/",
// MAGIC #    mount_point = '/mnt/cntexapure1',
// MAGIC #    extra_configs = {'fs.azure.account.key.stexapure.blob.core.windows.net':'a5uL2nMTajY7DKEZ8NiidhZr16dAMZGIum3CdvuIK5YRxKA88ovBo/SEOSgrOowGQzDETZw6CMFoHVj1JmNdwA=='})

// COMMAND ----------

// MAGIC %py
// MAGIC #silverdf = data.write.mode("append").format("csv").option("header",True).save("/mnt/cntexapure1")

// COMMAND ----------

// MAGIC %python
// MAGIC #%py
// MAGIC #dbutils.fs.unmount("/mnt/cntexapure")
...
savewriteWithDelta(df1, path, databaseName, tableName, schedule_date)
